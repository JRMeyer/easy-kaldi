...
nj=4   # this is the number of separate queue jobs we run, but each one
...
if [ -d "$dir" ]; then
    ...
    for f in $dir/final.ie \
                 $dir/*.ie \
                 $dir/final.mat \
                 $dir/final.dubm \
                 $dir/online_cmvn.conf \
                 $dir/global_cmvn.stats; do
        [ -f "$f" ] &&  mv $f ${bak_dir}/
    done
...
cp $srcdir/final.dubm \
   $srcdir/final.mat \
   $srcdir/global_cmvn.stats \
   $srcdir/splice_opts \
   $srcdir/online_cmvn.conf \
   $dir \
    || exit 1;
...
gmm_feats="ark,s,cs:apply-cmvn-online --config=$dir/online_cmvn.conf"
gmm_feats="$gmm_feats $dir/global_cmvn.stats scp:$sdata/JOB/feats.scp ark:- |"
gmm_feats="$gmm_feats splice-feats $splice_opts ark:- ark:- |"
gmm_feats="$gmm_feats transform-feats $dir/final.mat ark:- ark:- |"
gmm_feats="$gmm_feats subsample-feats --n=$subsample ark:- ark:- |"

feats="ark,s,cs:splice-feats $splice_opts scp:$sdata/JOB/feats.scp ark:- |"
...
if [ $stage -le -2 ]; then
    $cmd $dir/log/init.log \
         ...
         --use-weights=false \
         "gmm-global-to-fgmm $dir/final.dubm -|" \
         $dir/0.ie \
        || exit 1
fi
...
while [ $x -lt $num_iters ]; do
    if [ $stage -le $x ]; then
        ...
        echo "Accumulating stats (pass $x of $num_iters)"
        ...
        echo "Summing accs (pass $x of $num_iters)"
        ...
        echo "Updating model (pass $x of $num_iters)"

        # use the same number of threads that
