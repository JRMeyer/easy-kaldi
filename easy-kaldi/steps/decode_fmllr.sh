...
if [ $# != 5 ]; then
   ...
   echo "Usage: steps/decode_fmllr.sh [options] <graph-dir> <data-dir> <decode-dir> <unknown_phone> <silence_phone>"
   echo " e.g.: steps/decode_fmllr.sh exp/tri2b/graph_tgpr data/test_dev93 exp/tri2b/decode_dev93_tgpr SPOKEN_NOISE SIL"
...
if ! $skip_scoring ; then
    ...
    local/score.sh $scoring_opts --cmd "$cmd" \
        $data \
        $graphdir \
        $dir \
        $unknown_phone \
