...
# Make sure that the alignments provided were used to train the nnet!

# initial input nnet should not have been trained with mixingup
...
# Check data from each language, copy files to the right place
# Language index starts from 0.
for lang in $(seq 0 $[$num_lang-1]); do
    alidir[$lang]=${argv[$lang*2]}
    egs_dir[$lang]=${argv[$lang*2+1]}

    for f in ${egs_dir[$lang]}/info/frames_per_eg \
        ${egs_dir[lang]}/egs.1.ark \
        ${alidir[$lang]}/ali.1.gz \
        ${alidir[$lang]}/tree; do
...
if [ $stage -le -3 ]; then
    ...
    for lang in $(seq 0 $[$num_lang-1]); do
        $cmd $dir/$lang/log/train_trans.log \
            ...
            $dir/$lang/0.mdl \
            "ark:gunzip -c ${alidir[$lang]}/ali.*.gz|" \
            $dir/$lang/0.mdl \
            || exit 1;
    done
fi
...
if [ $num_threads -eq 1 ]; then
    # this enables us to use GPU code if we have just one thread.
    ...
    if [ $x -ge 0 ] && [ $stage -le $x ]; then
        ...
        for lang in $(seq 0 $[$num_lang-1]); do
            ...
            $cmd $dir/$lang/log/compute_prob_valid.$x.log \
                nnet-compute-prob \
                $dir/$lang/$x.mdl \
                ark:${egs_dir[$lang]}/valid_diagnostic.egs &
        ...
        (
            ...
            [ -f $dir/.error ] \
                && echo "$0: error averaging models on iteration $x of training" && \
                exit 1;
        ...
        for lang in $(seq 0 $[$num_lang-1]); do # mix up.
            ...
            [ ! -f $dir/$lang/$[$x+1].mdl ] \
                && echo "No such file $dir/$lang/$[$x+1].mdl" \
                && exit 1;
